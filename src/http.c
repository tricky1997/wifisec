...
void initHttp()
{
	...
	if (strcmp(buf, "(none)") == 0 ||
		...
		do_log(L_WARN,
		       "Couldn't determine host name -- using ``s_server''.\n");
		strcpy(buf, "s_server");
		goto success;
	}
...
int httpTimeoutHandler(TimeEventHandlerPtr event)
{
		       ...
		       ObjectPtr object, int from, int to)
{
	...
	n = httpPrintCacheControl(buf, n, len, object->cache_control, NULL);
	if (n < 0)
		goto fail;
...
static int cachePrintSeparator(char *buf, int offset, int len, int subsequent)
{
	int n = offset;
	if (subsequent)
...
HTTPConnectionPtr httpMakeConnection()
{
...
HTTPRequestPtr httpMakeRequest()
{
...
void httpQueueRequest(HTTPConnectionPtr connection, HTTPRequestPtr request)
{
...
int httpConnectionBigify(HTTPConnectionPtr connection)
{
	char *bigbuf;
...
int httpConnectionBigifyReqbuf(HTTPConnectionPtr connection)
{
	char *bigbuf;
...
int httpConnectionUnbigify(HTTPConnectionPtr connection)
{
	char *buf;
	assert(connection->flags & CONN_BIGBUF);
...
int httpConnectionUnbigifyReqbuf(HTTPConnectionPtr connection)
{
	char *buf;
	assert(connection->flags & CONN_BIGREQBUF);
...
HTTPConditionPtr httpMakeCondition()
{
...
int httpCondition(ObjectPtr object, HTTPConditionPtr condition)
{
		      ...
		      char *url, int url_len, char *etag)
{
	...
	if (code != 304) {
		...
		m = snnprintf(body, 0, CHUNK_SIZE,
			      ...
			      code, htmlMessage,
			      code >= 400 ?
			      "error occurred" : "status was returned");
		if (url_len > 0) {
	...
	if (headers)
		n = snnprint_n(buf, n, size, headers->string, headers->length);
...
AtomListPtr urlDecode(char *buf, int n)
{
	char mybuf[500];
	int i, j = 0;
	AtomListPtr list;
...
void httpTweakCachability(ObjectPtr object)
{
	...
	if (code != 200 && code != 206 &&
		...
		object->cache_control |=
		   ...
		   code != 300 && code != 301 && code != 304 && code != 410) {
		if (object->expires < 0
		    && !(object->cache_control & CACHE_PUBLIC)) {
