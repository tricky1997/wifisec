...
void releaseAtom(AtomPtr atom)
{
	...
	if (atom->refcount == 0) {
		int h = hash(0, atom->string, atom->length,
			     LOG2_ATOM_HASH_TABLE_SIZE);
...
static AtomPtr internAtomErrorV(int e, const char *f, va_list args)
{
