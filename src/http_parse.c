...
static int getNextTokenInList(const char *buf, int i,
			      ...
			      int *z_return, int *t_return, int *end_return);
...
void preinitHttpParser()
{
	CONFIG_VARIABLE_SETTABLE(censorReferer, CONFIG_TRISTATE,
				 configIntSetter, "Censor referer headers.");
...
#define A(name, value) name = internAtom(value); if(!name) goto fail;
	...
	A(atomXPolipoDate, "x-s_server-date");
	A(atomXPolipoAccess, "x-s_server-access");
	A(atomXPolipoLocation, "x-s_server-location");
	A(atomXPolipoBodyOffset, "x-s_server-body-offset");
#undef A
	return;
...
static int skipComment(const char *restrict buf, int i)
{
	assert(buf[i] == '(');
...
static int skipWhitespace(const char *restrict buf, int i)
{
	while (1) {
		if (buf[i] == ' ' || buf[i] == '\t')
			i++;
...
static int skipEol(const char *restrict buf, int i)
{
	while (buf[i] == ' ')
		i++;
	if (buf[i] == '\n')
...
static int skipToEol(const char *restrict buf, int i, int *start_return)
{
	while (buf[i] != '\n' && buf[i] != '\r')
		i++;
	if (buf[i] == '\n') {
...
static int parseInt(const char *restrict buf, int start, int *val_return)
{
	int i = start, val = 0;
	if (!digit(buf[i]))
		return -1;
...
static int
parseRange(const char *restrict buf, int i, int *from_return, int *to_return)
{
	int j;
	int from, to;
...
static int urlSameHost(const char *url1, int len1, const char *url2, int len2)
{
	int i;
	if (len1 < 7 || len2 < 7)
		return 0;
...
static char *resize_hbuf(char *hbuf, int *size, char *hbuf_small)
{
	int new_size = 2 * *size;
	char *new_hbuf;
...
httpParseHeaders(int client, AtomPtr url,
		 ...
		 time_t * expires_return, time_t * s_server_age_return,
		 time_t * s_server_access_return,
		 int *s_server_body_offset_return, int *age_return,
		 char **etag_return, AtomPtr * expect_return,
		 ...
		 AtomPtr * auth_return)
{
	...
	AtomPtr name = NULL;
	time_t date = -1, last_modified = -1, expires = -1, s_server_age = -1,
	    s_server_access = -1, s_server_body_offset = -1;
	int len = -1;
	...
	int persistent = (!request
			  || (request->connection->version != HTTP_10));
	char *location = NULL;
	AtomPtr via = NULL;
	AtomPtr auth = NULL;
	AtomPtr expect = NULL;
	...
	HTTPRangeRec range = { -1, -1, -1 }, content_range = {
	-1, -1, -1};
...
#define RESIZE_HBUF() \
	...
	while (1) {
		...
		name =
		    internAtomLowerN(buf + name_start, name_end - name_start);
		...
		if (name == atomConnection) {
			...
			while (1) {
				...
				if (token_compare
				    (buf, token_start, token_end, "close")) {
					persistent = 0;
				} else
				    if (token_compare
					(buf, token_start, token_end,
					 "keep-alive")) {
					persistent = 1;
				} else {
					if (hopToHop == NULL)
	...
	while (1) {
		...
		if (name == atomProxyConnection) {
			...
			goto fail;
			   ...
			   name == atomXPolipoDate
			   || name == atomXPolipoAccess) {
			time_t t;
			j = parse_time(buf, value_start, value_end, &t);
			if (j < 0) {
				if (name != atomExpires) {
					do_log(L_WARN, "Couldn't parse %s: ",
					       name->string);
					do_log_n(L_WARN, buf + value_start,
						 value_end - value_start);
					do_log(L_WARN, "\n");
			...
			if (j < 0) {
				...
				s_server_body_offset =
				    strtol(buf + value_start, &endptr, 10);
				if (endptr <= buf + value_start) {
					do_log(L_ERROR,
					       "Couldn't parse body offset.\n");
					goto fail;
			...
			if (token_compare
			    (buf, value_start, value_end, "identity"))
				te = TE_IDENTITY;
			else if (token_compare
				 (buf, value_start, value_end, "chunked"))
				te = TE_CHUNKED;
			else
				te = TE_UNKNOWN;
			...
			while (1) {
				...
				if (token_compare
				    (buf, token_start, token_end, "no-cache")) {
					cache_control.flags |= CACHE_NO;
				} else
				    if (token_compare
					(buf, token_start, token_end,
					...
					cache_control.flags |=
					    CACHE_NO_TRANSFORM;
				} else if (token_compare(buf, token_start, token_end, "must-revalidate") || token_compare(buf, token_start, token_end, "must-validate")) {	
					cache_control.flags |=
				...
				} else if (token_compare(buf, token_start, token_end, "max-age") || token_compare(buf, token_start, token_end, "maxage")) {	
					int a;
					if (v_start <= 0
					    || !digit(buf[v_start])) {
						do_log(L_WARN,
			...
			if (client) {
				j = parseRange(buf, value_start, &range.from,
					       &range.to);
				if (j < 0) {
					do_log(L_WARN,
			...
			if (name == atomVary) {
				if (!token_compare
				    (buf, value_start, value_end, "host")
				    && !token_compare(buf, value_start,
						      value_end, "*")) {
	...
	if (len_return)
		*len_return = len;
	if (cache_control_return)
		*cache_control_return = cache_control;
	if (condition_return) {
	...
	if (date_return)
		*date_return = date;
	if (last_modified_return)
		*last_modified_return = last_modified;
	if (expires_return)
		*expires_return = expires;
	if (s_server_age_return)
		*s_server_age_return = s_server_age;
	if (s_server_access_return)
		*s_server_access_return = s_server_access;
	if (s_server_body_offset_return)
		*s_server_body_offset_return = s_server_body_offset;
	if (age_return)
		*age_return = age;
	if (etag_return)
		*etag_return = etag;
	...
	if (range_return)
		*range_return = range;
	if (content_range_return)
		*content_range_return = content_range;
	if (location_return) {
...
{
	return (len >= 8 && memcmp(url, "/s_server/", 8) == 0);
}

int
parseChunkSize(const char *restrict buf, int i, int end, int *chunk_size_return)
{
	int v, d;
	v = h2i(buf[i]);
	if (v < 0)
...
int checkVia(AtomPtr name, AtomPtr via)
{
	int i;
	char *v;
