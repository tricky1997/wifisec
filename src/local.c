...
static void printConfig(FILE * out, char *dummy)
{
	(void)dummy;
	fprintf(out,
		"<!DOCTYPE HTML PUBLIC "
...
static void recursiveIndexDiskObjects(FILE * out, char *root)
{
	indexDiskObjects(out, root, 1);
}

static void plainIndexDiskObjects(FILE * out, char *root)
{
	indexDiskObjects(out, root, 0);
}
#endif

static void serversList(FILE * out, char *dummy)
{
	(void)dummy;
	listServers(out);
}

static int matchUrl(char *base, ObjectPtr object)
{
	int n = strlen(base);
