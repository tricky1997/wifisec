...
static ConfigVariablePtr findConfigVariable(AtomPtr name)
{
...
static void printString(FILE * out, char *string, int html)
{
	if (html) {
		char buf[512];
		int i;
...
printVariable(FILE * out, ConfigVariablePtr var, int html, int parseable)
{
	...
	switch (var->type) {
	case CONFIG_INT:
		fprintf(out, "%d", *var->value.i);
		break;
	case CONFIG_OCTAL:
		fprintf(out, "0%o", *var->value.i);
		break;
	case CONFIG_HEX:
		fprintf(out, "0x%x", *var->value.i);
		break;
	case CONFIG_TIME:
		{
			int v = *var->value.i;
			if (v == 0) {
				...
				if (v >= 3600 * 24)
					fprintf(out, "%dd", v / (3600 * 24));
				v = v % (3600 * 24);
				if (v >= 3600)
					fprintf(out, "%dh", v / 3600);
				v = v % 3600;
				if (v >= 60)
					fprintf(out, "%dm", v / 60);
				v = v % 60;
...
static void printVariableForm(FILE * out, ConfigVariablePtr var)
{
	char *disabled = "";
	unsigned int i;

	if (disableConfiguration || !var->setter)
	...
	case CONFIG_BOOLEAN:
		{
			...
			fprintf(out, "<select name=%s %s>", var->name->string,
				disabled);
			for (i = 0; i < sizeof(states) / sizeof(states[0]); i++) {
				if ((unsigned int)*var->value.i == i) {
					fprintf(out,
						"<option selected>%s</option>",
						states[i]);
				} else {
					fprintf(out, "<option>%s</option>",
						states[i]);
				}
			}
			fprintf(out, "</select>");
...
static int skipWhitespace(char *buf, int i)
{
...
static int parseInt(char *buf, int offset, int *value_return)
{
	char *p;
	int value;
...
static int parseState(char *buf, int offset, int kind)
{
	int i = offset;
	unsigned int n;
	int state = -1;
	...
	for (n = 0; n < sizeof(states) / sizeof(states[0]); n++) {
		if (strlen(states[n].name) == (unsigned int)i - offset &&
...
static int parseTime(char *line, int i, int *value_return)
{
	int v = 0, w;
	while (1) {
		if (!digit(line[i]))
...
int parseConfigFile(AtomPtr filename)
{
	char buf[512];
	int lineno;
	FILE *f;
...
int configIntSetter(ConfigVariablePtr var, void *value)
{
...
int configFloatSetter(ConfigVariablePtr var, void *value)
{
...
int configAtomSetter(ConfigVariablePtr var, void *value)
{
