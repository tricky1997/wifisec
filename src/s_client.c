...
static int httpAcceptAgain(TimeEventHandlerPtr event)
{
...
int httpAccept(int fd, FdEventHandlerPtr event, AcceptRequestPtr request)
{
	int rc;
...
void httpClientAbort(HTTPConnectionPtr connection, int closed)
{
...
static int httpClientDelayedShutdown(HTTPConnectionPtr connection)
{
...
static int httpClientDelayedShutdownHandler(TimeEventHandlerPtr event)
{
...
httpClientHandler(int status, FdEventHandlerPtr event, StreamRequestPtr request)
{
	...
	int bufsize =
	    (connection->
	     flags & CONN_BIGREQBUF) ? connection->reqlen : CHUNK_SIZE;
	...
	if (status != 0) {
		...
		httpClientAbort(connection, status > 0
				|| status == -ECONNRESET);
		connection->flags &= ~CONN_READER;
		return 1;
	}
...
httpClientRawError(HTTPConnectionPtr connection, int code, AtomPtr message,
		   int close)
{
	return httpClientRawErrorHeaders(connection, code, message, close,
					 NULL);
}
...
int httpClientNoticeError(HTTPRequestPtr request, int code, AtomPtr message)
{
	return httpClientNoticeErrorHeaders(request, code, message, NULL);
}

int httpClientError(HTTPRequestPtr request, int code, AtomPtr message)
{
	if (request->error_message)
		releaseAtom(request->error_message);
	request->error_code = code;
...
int httpClientLeanError(HTTPRequestPtr request, int code, AtomPtr message)
{
	if (request->error_message)
		releaseAtom(request->error_message);
	request->error_code = code;
			 ...
			 HTTPConnectionPtr connection)
{
	...
	if (connection->version != HTTP_UNKNOWN
...
static int httpClientRequestDelayed(TimeEventHandlerPtr event)
{
	...
	if (url == NULL) {
		...
		abortObject(request->object, 503,
			    internAtom("Couldn't allocate url"));
		return 1;
	}
...
int delayedHttpClientRequest(HTTPRequestPtr request)
{
			  ...
			  void *closure)
{
	...
	requestfn =
	    ...
	    httpLocalRequest : httpServerRequest;

	if (request->method == METHOD_POST || request->method == METHOD_PUT) {
		do {
			object =
			    findObject(OBJECT_HTTP, url->string, url->length);
			if (object) {
		...
		request->object =
		    makeObject(OBJECT_HTTP, url->string, url->length, 0, 0,
			       requestfn, NULL);
	...
	if (request->cache_control.flags & CACHE_AUTHORIZATION) {
		do {
			object =
			    makeObject(OBJECT_HTTP, url->string, url->length, 0,
				       0, requestfn, NULL);
		...
		if (!object)
			object =
			    makeObject(OBJECT_HTTP, url->string, url->length, 1,
				       1, requestfn, NULL);
	}
	releaseAtom(url);
	url = NULL;
...
int httpClientDiscardBody(HTTPConnectionPtr connection)
{
	...
	if (connection->reqlen > connection->reqbegin) {
		memmove(connection->reqbuf,
			connection->reqbuf + connection->reqbegin,
			connection->reqlen - connection->reqbegin);
		connection->reqlen -= connection->reqbegin;
		connection->reqbegin = 0;
	} else {
...
static int httpClientDelayed(TimeEventHandlerPtr event)
{
	HTTPConnectionPtr connection = *(HTTPConnectionPtr *) event->data;
...
int httpClientNoticeRequest(HTTPRequestPtr request, int novalidate)
{
	HTTPConnectionPtr connection = request->connection;
	...
	if (request->error_code) {
		if ((request->flags & REQUEST_FORCE_ERROR)
		    || REQUEST_SIDE(request) || request->object == NULL
		    || (request->object->flags & OBJECT_LOCAL)
	...
	if (serveNow) {
		...
		request->chandler =
		    conditionWait(&request->object->condition,
				  httpClientGetHandler, sizeof(request),
				  &request);
		if (request->chandler == NULL) {
			do_log(L_ERROR,
			       "Couldn't register condition handler.\n");
	...
	if (!mindlesslyCacheVary
	    && (request->object->cache_control & CACHE_VARY))
...
static int httpClientNoticeRequestDelayed(TimeEventHandlerPtr event)
{
	HTTPRequestPtr request = *(HTTPRequestPtr *) event->data;
	httpClientNoticeRequest(request, 0);
	return 1;
...
int delayedHttpClientNoticeRequest(HTTPRequestPtr request)
{
	TimeEventHandlerPtr event;
...
int httpClientContinueDelayed(TimeEventHandlerPtr event)
{
...
int delayedHttpClientContinue(HTTPConnectionPtr connection)
{
	TimeEventHandlerPtr event;
...
int httpClientGetHandler(int status, ConditionHandlerPtr chandler)
{
	...
	if ((object->flags & OBJECT_SUPERSEDED) &&
		...
		ObjectPtr new_object =
		    retainObject(request->request->can_mutate);
...
int httpClientSideRequest(HTTPRequestPtr request)
{
	...
	if (request->from < 0 || request->to >= 0) {
		httpClientNoticeError(request, 501,
				      internAtom
				      ("Partial requests not implemented"));
		httpClientDiscardBody(connection);
		return 1;
	}
	if (connection->reqte != TE_IDENTITY) {
		httpClientNoticeError(request, 501,
				      internAtom
				      ("Chunked requests not implemented"));
		httpClientDiscardBody(connection);
		return 1;
	}
	if (connection->bodylen < 0) {
		      ...
		      FdEventHandlerPtr event, StreamRequestPtr srequest)
{
	...
	if (request->request) {
		...
		pokeFdEvent(request->request->connection->fd, -ESHUTDOWN,
			    POLLOUT);
	}
	notifyObject(request->object);
...
int httpServeObject(HTTPConnectionPtr connection)
{
	HTTPRequestPtr request = connection->request;
	ObjectPtr object = request->object;
...
static int httpServeObjectDelayed(TimeEventHandlerPtr event)
{
	HTTPConnectionPtr connection = *(HTTPConnectionPtr *) event->data;
...
int httpServeChunk(HTTPConnectionPtr connection)
{
	HTTPRequestPtr request = connection->request;
	ObjectPtr object = request->object;
	...
	if (len <= 0) {
		...
		if (!(object->flags & OBJECT_INPROGRESS)
			...
			if (object->chunks[i].size < CHUNK_SIZE && to >= 0
			    && connection->offset + len + 1 < to)
				object->request(object, request->method,
						connection->offset + len, -1,
						request,
						object->request_closure);
			else if (i + 1 < object->numchunks
				 && object->chunks[i + 1].size == 0 && to >= 0
				 && (i + 1) * CHUNK_SIZE + 1 < to)
				object->request(object, request->method,
						(i + 1) * CHUNK_SIZE, -1,
						request,
		...
		if (len2 == 0) {
			...
			do_stream(IO_WRITE | IO_NOTNOW |
				  (connection->te ==
				   TE_CHUNKED ? IO_CHUNKED : 0) | (end ? IO_END
								   : 0),
				  connection->fd, 0, object->chunks[i].data + j,
				  len, httpServeObjectStreamHandler,
				  connection);
		} else {
			httpSetTimeout(connection, clientTimeout);
			do_log(D_CLIENT_DATA,
...
static int httpServeChunkDelayed(TimeEventHandlerPtr event)
{
	HTTPConnectionPtr connection = *(HTTPConnectionPtr *) event->data;
...
int delayedHttpServeChunk(HTTPConnectionPtr connection)
{
	TimeEventHandlerPtr event;
...
int httpServeObjectHandler(int status, ConditionHandlerPtr chandler)
{
				   ...
				   StreamRequestPtr srequest)
{
	...
	HTTPRequestPtr request = connection->request;
	int condition_result =
	    httpCondition(request->object, request->condition);
	int i = connection->offset / CHUNK_SIZE;
