...
void preinitObject()
{
	...
	CONFIG_VARIABLE(objectHighMark, CONFIG_INT, "High object count mark.");
	...
	CONFIG_VARIABLE_SETTABLE(maxAgeFraction, CONFIG_FLOAT,
				 configFloatSetter,
	...
	CONFIG_VARIABLE_SETTABLE(dontCacheCookies, CONFIG_BOOLEAN,
				 configIntSetter,
...
void initObject()
{
	...
	q = 0;
	if (publicObjectLowMark == 0)
		q = 1;
	if (publicObjectLowMark < 8
	    || publicObjectLowMark >= objectHighMark - 4) {
	   ...
	   RequestFunction request, void *request_closure)
{
	...
	if (publicObjectCount >= publicObjectLowMark && !objectExpiryScheduled) {
	...
	if (public) {
		...
		if (objectHashTable[h]) {
			writeoutToDisk(objectHashTable[h],
				       objectHashTable[h]->size, -1);
...
void objectMetadataChanged(ObjectPtr object, int revalidate)
{
	if (revalidate) {
...
ObjectPtr retainObject(ObjectPtr object)
{
...
void releaseNotifyObject(ObjectPtr object)
{
	do_log(D_REFCOUNT, "O 0x%lx %d--\n",
	       (unsigned long)object, object->refcount);
	object->refcount--;
...
void lockChunk(ObjectPtr object, int i)
{
...
void unlockChunk(ObjectPtr object, int i)
{
...
int objectSetChunks(ObjectPtr object, int numchunks)
{
	...
	if (object->length >= 0)
		n = MAX(numchunks,
			(object->length + (CHUNK_SIZE - 1)) / CHUNK_SIZE);
	else
		n = MAX(numchunks,
...
objectAddChunkEnd(ObjectPtr object, const char *data, int offset, int plen)
{
	...
	memcpy(object->chunks[i].data + (offset % CHUNK_SIZE), data, plen);
...
int objectAddData(ObjectPtr object, const char *data, int offset, int len)
{
	...
	if (offset + len >= object->numchunks * CHUNK_SIZE) {
		rc = objectSetChunks(object,
				     (offset + len - 1) / CHUNK_SIZE + 1);
		if (rc < 0) {
			return -1;
		}
	}
...
void objectPrintf(ObjectPtr object, int offset, const char *format, ...)
{
	...
	if (buf == NULL) {
		abortObject(object, 500,
			    internAtom("Couldn't allocate string"));
		return;
	}
...
int objectHoleSize(ObjectPtr object, int offset)
{
	...
	if (offset % CHUNK_SIZE != 0) {
		if (object->chunks[offset / CHUNK_SIZE].size >
		    offset % CHUNK_SIZE)
			return 0;
		else {
...
void destroyObject(ObjectPtr object)
{
	...
	if (object->flags & OBJECT_PUBLIC) {
		...
		if (object->key)
			free(object->key);
		if (object->headers)
			releaseAtom(object->headers);
		if (object->etag)
			free(object->etag);
		if (object->via)
			releaseAtom(object->via);
...
void abortObject(ObjectPtr object, int code, AtomPtr message)
{
	int i;
...
void supersedeObject(ObjectPtr object)
{
...
void notifyObject(ObjectPtr object)
{
	retainObject(object);
...
int discardObjectsHandler(TimeEventHandlerPtr event)
{
	(void)event;
	return discardObjects(0, 0);
}

void writeoutObjects(int all)
{
	...
	while (object) {
		do {
			...
			n = writeoutToDisk(object, -1,
					   all ? -1 : maxWriteoutWhenIdle);
			bytes += n;
	...
	if (all || force || used_chunks >= CHUNKS(chunkHighMark) ||
		...
		if (force || used_chunks > CHUNKS(chunkCriticalMark)) {
			...
			while (object &&
				...
				if (force || (object->flags & OBJECT_PUBLIC)) {
					int j;
					for (j = object->numchunks - 1; j >= 0;
					     j--) {
						if (object->chunks[j].locked)
							continue;
						if (object->chunks[j].size <
						    CHUNK_SIZE)
							continue;
						writeoutToDisk(object,
							       (j +
...
int objectIsStale(ObjectPtr object, CacheControlPtr cache_control)
{
	...
	if (cache_control->s_maxage >= 0) {
		if (object->s_maxage >= 0)
			s_maxage =
			    MIN(cache_control->s_maxage, object->s_maxage);
		else
