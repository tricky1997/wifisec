...
void preinitServer(void)
{
	...
	CONFIG_VARIABLE_SETTABLE(replyUnpipelineTime, CONFIG_TIME,
				 configIntSetter,
				 "Estimated time for a pipeline break.");
	CONFIG_VARIABLE_SETTABLE(replyUnpipelineSize, CONFIG_INT,
				 configIntSetter, "Size for a pipeline break.");
	CONFIG_VARIABLE_SETTABLE(pipelineAdditionalRequests, CONFIG_TRISTATE,
				 configIntSetter,
				 "Pipeline requests on an active connection.");
	CONFIG_VARIABLE_SETTABLE(maxPipelineTrain, CONFIG_INT, configIntSetter,
				 "Maximum number of requests "
				 "pipelined at a time.");
	CONFIG_VARIABLE(pmmFirstSize, CONFIG_INT,
			"The size of the first PMM chunk.");
	CONFIG_VARIABLE(pmmSize, CONFIG_INT, "The size of a PMM chunk.");
...
static void discardServer(HTTPServerPtr server)
{
...
static int httpServerIdle(HTTPServerPtr server)
{
	int i;
	if (server->request)
		return 0;
...
static int expireServersHandler(TimeEventHandlerPtr event)
{
...
void initServer(void)
{
	...
	if (pmmFirstSize || pmmSize) {
		if (pmmSize == 0)
			pmmSize = pmmFirstSize;
		if (pmmFirstSize == 0)
			pmmFirstSize = pmmSize;
	...
	event =
	    scheduleTimeEvent(serverExpireTime / 60 + 60, expireServersHandler,
			      0, NULL);
	if (event == NULL) {
		do_log(L_ERROR, "Couldn't schedule server expiry.\n");
		exit(1);
...
static HTTPServerPtr getServer(char *name, int port, int proxy)
{
	...
	server->idleHandler =
	    malloc(serverMaxSlots * sizeof(FdEventHandlerPtr));
	if (server->connection == NULL) {
		do_log(L_ERROR, "Couldn't allocate server.\n");
...
void httpServerClientReset(HTTPRequestPtr request)
{
	if (request->connection &&
		...
		pokeFdEvent(request->connection->fd, -ECLIENTRESET,
			    POLLIN | POLLOUT);
}
		      ...
		      int method, int from, int to, HTTPRequestPtr requestor)
{
	...
	if (from % CHUNK_SIZE != 0) {
		if (allowUnalignedRangeRequests) {
			objectFillFromDisk(object,
					   from / CHUNK_SIZE * CHUNK_SIZE, 1);
...
int httpServerConnection(HTTPServerPtr server)
{
	...
	do_gethostbyname(server->name, 0,
			 httpServerConnectionDnsHandler, connection);
...
int httpServerConnectionDnsHandler(int status, GethostbynameRequestPtr request)
{
	...
	if (status <= 0) {
		...
		if (connection->server->request)
			httpServerAbortRequest(connection->server->request, 1,
					       504, retainAtom(message));
	...
	if (request->addr->string[0] == DNS_CNAME) {
		if (request->count > 10) {
			...
			connection->connecting = 0;
			if (connection->server->request)
				httpServerAbortRequest(connection->
						       server->request, 1, 504,
						       retainAtom(message));
...
httpServerConnectionHandler(int status,
			    FdEventHandlerPtr event, ConnectRequestPtr request)
{
	HTTPConnectionPtr connection = request->data;
	...
	if (status < 0) {
		...
		if (connection->server->request)
			httpServerAbortRequest(connection->server->request,
					       ...
					       retainAtom(message));
		httpServerAbort(connection, status != -ECLIENTRESET, 504,
				message);
		return 1;
	}
...
int httpServerIdleHandler(int a, FdEventHandlerPtr event)
{
...
static void httpServerDiscardRequests(HTTPServerPtr server)
{
	HTTPRequestPtr request;
	while (server->request && !server->request->request) {
		...
		if (server->request == NULL)
			server->request_last = NULL;
		request->object->flags &=
		    ~(OBJECT_INPROGRESS | OBJECT_VALIDATING);
...
static int pipelineIsSmall(HTTPConnectionPtr connection)
{
	HTTPRequestPtr request = connection->request;
...
static int numRequests(HTTPServerPtr server)
{
	int n = 0;
...
httpServerGetConnection(HTTPServerPtr server, int *idle_return)
{
	...
	if (empty) {
		if ((connecting == 0
		     || (server->persistent <= 0 && connecting <= 1))
	...
	if (server->version == HTTP_11 && server->pipeline >= 4) {
		for (i = 0; i < serverSlots; i++) {
			if (server->connection[i]
				...
				if (server->idleHandler[i])
					unregisterFdEvent(server->idleHandler
							  [i]);
				server->idleHandler[i] = NULL;
				*idle_return = 0;
...
int httpServerTrigger(HTTPServerPtr server)
{
	...
	while (server->request) {
		...
		while (server->request && connection->pipelined < n) {
			...
			if (rc < 0) {
				if (i == 0)
					httpServerAbortRequest(request,
							       rc !=
							       -ECLIENTRESET,
							       503,
							       internAtom
		...
		if (i > 0)
			httpServerSendRequest(connection);
	...
	for (i = 0; i < server->maxslots; i++) {
		if (server->connection[i] &&
			...
			if (!server->idleHandler[i])
				server->idleHandler[i] =
				    registerFdEvent(server->connection[i]->fd,
						    POLLIN,
			...
			httpSetTimeout(server->connection[i],
				       serverIdleTimeout);
		}
	}
...
static int httpClientDelayedDoSideHandler(TimeEventHandlerPtr event)
{
	HTTPConnectionPtr connection = *(HTTPConnectionPtr *) event->data;
	httpServerDoSide(connection);
	return 1;
...
static int httpServerDelayedDoSide(HTTPConnectionPtr connection)
{
			    ...
			    FdEventHandlerPtr event, StreamRequestPtr srequest)
{
	...
	if (client->reqlen > client->reqbegin + bodylen)
		memmove(client->reqbuf,
			client->reqbuf + client->reqbegin + bodylen,
...
void httpServerFinish(HTTPConnectionPtr connection, int s, int offset)
{
	...
	if (request) {
		...
		if (request->time1.tv_sec != null_time.tv_sec) {
			...
			if (!(request->flags & REQUEST_PIPELINED) &&
				...
				rtt =
				    timeval_minus_usec(&request->time1,
						       &request->time0);
			if (size >= 8192 && d > 50000)
				rate =
				    ((double)size / (double)d) * 1000000.0 +
				    0.5;
		}
		request->time0 = null_time;
		request->time1 = null_time;
	...
	if (s) {
		...
		if (server->persistent < -5)
			server->numslots =
			    MIN(server->maxslots, serverMaxSlots);
...
static int httpServerDelayedFinishHandler(TimeEventHandlerPtr event)
{
	HTTPConnectionPtr connection = *(HTTPConnectionPtr *) event->data;
	httpServerFinish(connection, 1, 0);
	return 1;
...
static void httpServerDelayedFinish(HTTPConnectionPtr connection)
{
	TimeEventHandlerPtr handler;
...
int httpConnectionPipelined(HTTPConnectionPtr connection)
{
	HTTPRequestPtr request = connection->request;
	int i = 0;
	while (request) {
...
void httpServerRestart(HTTPConnectionPtr connection)
{
	HTTPServerPtr server = connection->server;
	HTTPRequestPtr request = connection->request;
		  ...
		  HTTPRequestPtr requestor, void *closure)
{
	...
	if (rc < 0) {
		abortObject(object,
			    503,
			    internAtom("Couldn't schedule server request"));
		notifyObject(object);
		return 1;
	}
		 ...
		 int bodylen)
{
	...
	if (request->method == METHOD_GET ||
		...
		if (object->flags & OBJECT_DYNAMIC) {
			...
			if (pmmSize && connection->server->pipeline >= 4) {
				if (from == 0)
					to = to < 0 ? pmmFirstSize : MIN(to,
									 pmmFirstSize);
				else
					to = to < 0 ? from + pmmSize : MIN(to,
									   from
									   +
									   pmmSize);
			}
	...
	if (connection->server->isProxy) {
		n = snnprint_n(connection->reqbuf, n, bufsize, url, url_size);
	} else {
		if (url_size - z == 0) {
			location = "/";
			location_size = 1;
	...
	if (!disableVia) {
		if (request->request && request->request->via) {
			n = snnprintf(connection->reqbuf, n, bufsize,
				      "\r\nVia: %s, 1.1 %s",
				      request->request->via->string,
				      proxyName->string);
		} else {
			n = snnprintf(connection->reqbuf, n, bufsize,
				      "\r\nVia: 1.1 %s", proxyName->string);
		}
	}
		       ...
		       FdEventHandlerPtr event, StreamRequestPtr srequest)
{
	...
	if (i >= 0) {
		...
		return httpServerHandlerHeaders(status, event, srequest,
						connection);
	}
	...
	if (status) {
		...
		if (status < 0) {
			...
			httpServerAbort(connection, status != -ECLIENTRESET,
					502, internAtomError(-status,
			 ...
			 HTTPConnectionPtr connection)
{
	...
	rc = httpParseServerFirstLine(connection->buf, &code, &version,
				      &message);
	if (rc <= 0) {
	...
	if (code == 100) {
		...
		connection->len -= rc;
		if (connection->len > 0)
			memmove(connection->buf, connection->buf + rc,
				connection->len);
		httpServerReply(connection, 1);
		return 1;
	}
	...
	if (via && !checkVia(proxyName, via)) {
		httpServerAbort(connection, 1, 504,
	...
	if (code == 412) {
		if (request->method != METHOD_CONDITIONAL_GET ||
			...
			httpServerAbort(connection, 1, 502,
					internAtom
					("Unexpected \"precondition failed\" "
					 "reply from server"));
			goto fail;
		}
	}
	...
	if (code == 304 || code == 412) {
		...
		if (!supersede && (object->cache_control & CACHE_VARY) &&
			...
			if (!httpHeaderMatch
			    (atomContentType, object->headers, headers)
			    || !httpHeaderMatch(atomContentEncoding,
		...
		if (full_len < 0 && te == TE_IDENTITY) {
			...
			if (expect_body && content_range.from < 0
			    && content_range.to < 0)
				supersede = 1;
		}
		...
		httpServerAbort(connection, 1, 502,
				internAtom
				("Unexpected \"range not satisfiable\" "
				 "reply"));
	...
	case 405:
	case 401:
		if (new_object->message)
			releaseAtom(new_object->message);
			...
			if (rc < 0) {
				...
				if (code != 206) {
					if (object->length < 0) {
						...
						objectMetadataChanged(object,
								      0);
					} else if (object->length !=
						   object->size) {
						httpServerAbort(connection, 1,
								500,
								internAtom
								("Inconsistent "
								 "object size"));
...
int httpServerIndirectHandlerCommon(HTTPConnectionPtr connection, int eof)
{
	HTTPRequestPtr request = connection->request;
	...
	if (eof && connection->len == 0) {
		if (connection->te == TE_CHUNKED ||
			...
			if (request->object->length < 0 && eof > 0 &&
			    (request->to < 0
			     || request->to > request->object->size)) {
...
int httpServerReadData(HTTPConnectionPtr connection, int immediate)
{
	HTTPRequestPtr request = connection->request;
	ObjectPtr object = request->object;
	int to = -1;
				...
				if (object->chunks[i + 1].data) {
					do_stream_3(IO_READ | IO_NOTNOW,
						    ...
						    MIN(CHUNK_SIZE,
							       ...
							       1) * CHUNK_SIZE),
						    connection->buf,
						    connection->buf ? more : 0,
						    httpServerDirectHandler2,
						    connection);
					return 1;
				}
			      ...
			      StreamRequestPtr srequest)
{
	...
	end1 =
	    MIN(end, i * CHUNK_SIZE + MIN(kind * CHUNK_SIZE, srequest->offset));

	assert(end >= 0 && end1 >= i * CHUNK_SIZE
	       && end1 <= (i + 2) * CHUNK_SIZE);
...
int connectionAddData(HTTPConnectionPtr connection, int skip)
{
	...
	if (connection->te == TE_IDENTITY) {
		...
		if ((object->length >= 0
		     && object->length <= connection->offset)
		...
		connection->len -= i;
		if (connection->len > 0)
			memmove(connection->buf, connection->buf + i,
				connection->len);
...
void listServers(FILE * out)
{
	...
	fprintf(out, "<thead><tr><th>Server</th>"
		...
		"<th>Pipeline</th>"
		"<th>Connections</th>"
		"<th></th>" "<th>rtt</th>" "<th>rate</th>" "</tr></thead>\n");
	fprintf(out, "<tbody>\n");
	server = servers;
	entry = 0;
	while (server) {
		...
		if (server->port == 80)
			...
			fprintf(out, "<td>%s:%d</td>", server->name,
				server->port);
		...
		if (server->lies > 0)
			fprintf(out, "<td>(%d lies)</td>",
				(server->lies + 9) / 10);
		else
			fprintf(out, "<td></td>");

		if (server->rtt > 0)
			fprintf(out, "<td>%.3f</td>",
				(double)server->rtt / 1000000.0);
		else
			fprintf(out, "<td></td>");
		if (server->rate > 0)
	...
	fprintf(out, "</tbody>\n");
	fprintf(out, "</table>\n");
	fprintf(out, "<p><a href=\"/s_server/\">back</a></p>");
