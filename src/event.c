...
static inline int timeval_cmp(struct timeval *t1, struct timeval *t2)
{
...
void timeToSleep(struct timeval *time)
{
	if (!timeEventQueue) {
		time->tv_sec = ~0L;
		time->tv_usec = ~0L;
...
static TimeEventHandlerPtr enqueueTimeEvent(TimeEventHandlerPtr event)
{
	...
	if (timeEventQueue == NULL ||
		...
		while (otherevent->previous &&
		       timeval_cmp(&event->time,
				   &otherevent->previous->time) < 0) {
...
int allocateFdEventNum(int fd)
{
	...
	if (fdEventNum < fdEventSize) {
		...
		new_poll_fds =
		    realloc(poll_fds, new_size * sizeof(struct pollfd));
		if (!new_poll_fds)
			return -1;
		new_fdEvents =
		    realloc(fdEvents, new_size * sizeof(FdEventHandlerPtr));
		if (!new_fdEvents)
			return -1;
...
static int recomputePollEvents(FdEventHandlerPtr event)
{
	int pe = 0;
	while (event) {
...
static void unregisterFdEventI(FdEventHandlerPtr event, int i)
{
...
static FdEventHandlerPtr findEventHelper(int revents, FdEventHandlerPtr events)
{
...
static FdEventHandlerPtr findEvent(int revents, FdEventHandlerPtr events)
{
	FdEventHandlerPtr event;
...
static int pokeFdEventHandler(TimeEventHandlerPtr tevent)
{
...
void pokeFdEvent(int fd, int status, int what)
{
	...
	handler = scheduleTimeEvent(0, pokeFdEventHandler, sizeof(poke), &poke);
	if (!handler) {
	...
	while (1) {
		...
		if (sleep_time.tv_sec == -1) {
			...
			if (timeval_cmp(&sleep_time, &current_time) <= 0) {
				...
				int t;
				timeval_minus(&timeout, &sleep_time,
					      &current_time);
				t = timeout.tv_sec * 1000 + (timeout.tv_usec +
							     999) / 1000;
				rc = poll(poll_fds, fdEventNum,
					  diskIsClean ? t : MIN(idleTime * 1000,
								t));
			}
		}
		...
		fd0 =
		    (current_time.tv_usec ^ (current_time.tv_usec >> 16)) %
		    fdEventNum;
		n = rc;
		for (i = 0; i < fdEventNum; i++) {
			...
			if (poll_fds[j].revents) {
				n--;
				event =
				    findEvent(poll_fds[j].revents, fdEvents[j]);
				if (!event)
					continue;
...
void initCondition(ConditionPtr condition)
{
...
void unregisterConditionHandler(ConditionHandlerPtr handler)
{
...
void abortConditionHandler(ConditionHandlerPtr handler)
{
	int done;
