...
do_stream(int operation, int fd, int offset, char *buf, int len,
	  int (*handler) (int, FdEventHandlerPtr, StreamRequestPtr), void *data)
{
	      ...
	      void *data)
{
	...
	return schedule_stream(operation, fd, offset,
			       ...
			       NULL, 0, NULL, 0, buf_location, handler, data);
}

static int chunkHeaderLen(int i)
{
	if (i <= 0)
		return 0;
	if (i < 0x10)
...
static int chunkHeader(char *buf, int buflen, int i)
{
	int n;
	if (i <= 0)
		return 0;
...
int do_scheduled_stream(int status, FdEventHandlerPtr event)
{
	...
	if (request->offset < 0) {
		assert((request->operation &
			(IO_MASK | IO_BUF3 | IO_BUF_LOCATION)) == IO_WRITE);
		...
		if (request->offset < -chunk_header_len) {
			assert(request->offset >=
			       -(request->u.h.hlen + chunk_header_len));
	...
	if (request->len > 0) {
		if (request->buf == NULL &&
			...
			if (request->buf == NULL) {
				    ...
				    request->handler(-ENOMEM, event, request);
				return done;
			}
		}
		if (request->offset <= 0) {
	...
	if (request->len2 > 0) {
		if (request->offset <= request->len) {
			...
			iov[i].iov_base =
			    request->buf2 + request->offset - request->len;
			iov[i].iov_len =
			    request->len2 - request->offset + request->len;
			i++;
		}
	}
	...
	if ((request->operation & IO_BUF3) && request->u.b.len3 > 0) {
		if (request->offset <= len12) {
			...
			iov[i].iov_base =
			    request->u.b.buf3 + request->offset - len12;
			iov[i].iov_len =
			    request->u.b.len3 - request->offset + len12;
			i++;
		}
	}
	...
	if ((request->operation & IO_MASK) == IO_WRITE) {
		if (i > 1)
			...
			rc = WRITE(request->fd, iov[0].iov_base,
				   iov[0].iov_len);
	} else {
		if (i > 1)
...
int streamRequestDone(StreamRequestPtr request)
{
	int len123 =
	    request->len + request->len2 +
	    ((request->operation & IO_BUF3) ? request->u.b.len3 : 0);
...
static int serverSocket(int af)
{
	int fd, rc;
	if (af == 4) {
	   ...
	   void *data)
{
	...
	if ((unsigned int)index >= (addr->length - 1) / sizeof(HostAddressRec))
		index = 0;
...
int do_scheduled_connect(int status, FdEventHandlerPtr event)
{
	...
	assert(addr->length > 0 && addr->string[0] == DNS_A);
	assert(addr->length % sizeof(HostAddressRec) == 1);
	assert((unsigned int)request->index <
	       (addr->length - 1) / sizeof(HostAddressRec));
	...
	case 6:
		...
		memcpy(&servaddr6.sin6_addr, &host->data,
		       sizeof(struct in6_addr));
		rc = connect(request->fd, (struct sockaddr *)&servaddr6,
			     sizeof(servaddr6));
#else
		rc = -1;
		errno = EAFNOSUPPORT;
...
int do_scheduled_accept(int status, FdEventHandlerPtr event)
{
...
int setV6only(int fd, int v6only)
{
	int val = v6only ? 1 : 0;
	int rc;
	rc = setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&val,
			sizeof(val));
	if (rc < 0)
		return -1;
	return 0;
...
static int lingeringCloseTimeoutHandler(TimeEventHandlerPtr event)
{
...
static int lingeringCloseHandler(int status, FdEventHandlerPtr event)
{
	LingeringClosePtr l = *(LingeringClosePtr *) event->data;
	char buf[17];
	int rc;
...
static int bitmatch(const unsigned char *a, const unsigned char *b, int n)
{
	if (n >= 8) {
		if (memcmp(a, b, n / 8) != 0)
			return 0;
...
static int match(int af, unsigned char *data, NetAddressPtr list)
{
	int i;
#ifdef HAVE_IPv6
