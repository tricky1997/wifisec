...
static char *mkfilename(const char *path, char *filename)
{
	int n = strlen(path);
...
static int split(const char *path, int *slash_return, int *dlen, int *blen)
{
	int len;
	int slash;
	len = strlen(path);
...
static char *basename_a(const char *path)
{
	int blen, slash;
	char *b;
	int rc;
...
static char *dirname_a(const char *path)
{
	int dlen;
	int rc;
	char *d;
